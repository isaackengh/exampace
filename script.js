...
document.getElementById("timeInput").addEventListener("input", function () {
	let timeInput = parseFloat(this.value);
	if (!isNaN(timeInput) && timeInput > 0) {
		let totalSeconds = Math.floor(timeInput * 1.2 * 60);
		let minutes = Math.floor(totalSeconds / 60);
		let seconds = totalSeconds % 60;
		document.getElementById("timerDisplay").innerText = `${minutes < 10 ? "0" : ""}${minutes}:${
			seconds < 10 ? "0" : ""
		}${seconds}`;
		document.getElementById("startButton").disabled = false; // Enable the "Start Timer" button
	} else {
